...
do
    ...
    go test -shuffle=on ${race:-} -timeout="${TIMEOUT:-600s}" -coverprofile=coverage.out -covermode=atomic "$@" $(go list github.com/ava-labs/subnet-evm/... | grep -v github.com/ava-labs/subnet-evm/tests) | tee test.out || command_status=$?
