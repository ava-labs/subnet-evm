...
extra_imports=$(grep -r --include='*.go' --exclude-dir='simulator' ${geth_regexp} -h | grep -v "${allow_named_imports}" | grep -o ${geth_regexp} | sort -u | comm -23 - ./scripts/geth-allowed-packages.txt)
