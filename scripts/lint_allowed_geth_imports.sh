...
extra_imports=$(grep -r --include='*.go' --exclude-dir='simulator' --exclude-dir='coreth' '"github.com/ethereum/go-ethereum/.*"' -o -h | sort -u | comm -23 - ./scripts/geth-allowed-packages.txt)
...
extra_imports=$(grep -r --include='*.go' --exclude-dir='coreth' '"github.com/ava-labs/coreth/.*"' -o -h || true | sort -u)
if [ ! -z "${extra_imports}" ]; then
