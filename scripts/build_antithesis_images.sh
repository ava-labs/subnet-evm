...
build_antithesis_builder_image "${GO_VERSION}" "antithesis-subnet-evm-builder:${IMAGE_TAG}" "${AVALANCHEGO_CLONE_PATH}" "${SUBNET_EVM_PATH}" "avalanche"
...
gen_antithesis_compose_config "${IMAGE_TAG}" "${SUBNET_EVM_PATH}/avalanche/tests/antithesis/gencomposeconfig" \
...
build_antithesis_images "${GO_VERSION}" "${IMAGE_PREFIX}" "antithesis-subnet-evm" "${IMAGE_TAG}" \
                        "${AVALANCHEGO_IMAGE_TAG}" "${SUBNET_EVM_PATH}/avalanche/tests/antithesis/Dockerfile" \
