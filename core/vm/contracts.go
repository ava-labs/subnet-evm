...
func NewCreateClient(accessibleState contract.AccessibleState) *CreateClient {
	return &CreateClient{
...
func (c *CreateClient) Run(input []byte) ([]byte, error) {
	/*
		input
	...
	*/
	b := c.accessibleState.GetStateDB().GetPrecompileState(common.BytesToAddress([]byte("nextClientSeq")))
	...
	c.accessibleState.GetStateDB().SetPrecompileState(common.BytesToAddress([]byte("nextClientSeq")), b)
	...
	c.accessibleState.GetStateDB().SetPrecompileState(common.BytesToAddress([]byte(clientStatePath)), clientStateByte)

	// store ConsensusStateBytes
	consensusStatePath := fmt.Sprintf("clients/%s/consensusStates/%s", clientID, clientState.GetLatestHeight())
	c.accessibleState.GetStateDB().SetPrecompileState(common.BytesToAddress([]byte(consensusStatePath)), consensusStateByte)
