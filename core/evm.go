...
func NewEVMBlockContext(header *types.Header, chain ChainContext, author *common.Address) (vm.BlockContext, error) {
	var (
	...
	if len(header.Extra) > params.DynamicFeeExtraDataSize {
		...
		if err != nil {
			return vm.BlockContext{}, fmt.Errorf("failed to parse predicate results: %w", err)
		}
	}

	return newEVMBlockContext(header, chain, author, predicateResults), nil
}
