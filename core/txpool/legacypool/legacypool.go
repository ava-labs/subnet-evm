...
func (pool *LegacyPool) Pending(filter txpool.PendingFilter) map[common.Address][]*txpool.LazyTransaction {
	return pool.PendingWithBaseFee(filter, nil)
}

// If baseFee is nil, then pool.priced.urgent.baseFee is used.
func (pool *LegacyPool) PendingWithBaseFee(filter txpool.PendingFilter, baseFee *big.Int) map[common.Address][]*txpool.LazyTransaction {
	...
	for addr, list := range pool.pending {
		...
		if minTipBig != nil && !pool.locals.contains(addr) {
			for i, tx := range txs {
				if tx.EffectiveGasTipIntCmp(pool.gasTip.Load().ToBig(), baseFeeBig) < 0 {
					txs = txs[:i]
					break
				}
			}
