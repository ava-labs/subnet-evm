...
func CheckTransactionPredicates(rules params.Rules, snowCtx *snow.Context, txs types.Transactions, proposerVMBlockCtx *block.Context) (int, error) {
...
func (s *StateDB) PrepareAccessList(sender common.Address, dst *common.Address, precompiles []common.Address, list types.AccessList) {
	...
	for _, el := range list {
		...
		storageKeysBytes := utils.HashSliceToBytes(el.StorageKeys)
		for _, key := range el.StorageKeys {
			s.AddSlotToAccessList(el.Address, key)
		}
		predicateStorageSlots[el.Address] = storageKeysBytes
	}

	// Set predicateStorageSlots once, not to be modified
	s.predicateStorageSlots = predicateStorageSlots
}
