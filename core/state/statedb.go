...
func (s *StateDB) commitAndFlush(block uint64, deleteEmptyObjects bool, snaps *snapshot.Tree, blockHash, parentHash common.Hash, referenceRoot bool) (*stateUpdate, error) {
	...
	if !ret.empty() {
		...
		if db := s.db.TrieDB(); db != nil {
			...
			if referenceRoot {
				if err := db.UpdateAndReferenceRoot(ret.root, ret.originRoot, block, ret.nodes, set); err != nil {
					return nil, err
				}
			} else {
...
func (s *StateDB) CommitWithSnap(block uint64, deleteEmptyObjects bool, snaps *snapshot.Tree, blockHash, parentHash common.Hash, referenceRoot bool) (common.Hash, error) {
	ret, err := s.commitAndFlush(block, deleteEmptyObjects, snaps, blockHash, parentHash, referenceRoot)
	if err != nil {
...
func (s *StateDB) Commit(block uint64, deleteEmptyObjects bool, referenceRoot bool) (common.Hash, error) {
	ret, err := s.commitAndFlush(block, deleteEmptyObjects, nil, common.Hash{}, common.Hash{}, referenceRoot)
	if err != nil {
		return common.Hash{}, err
	}
	return ret.root, nil
