...
const testFn = (fnNameOrNames: string | string[], overrides = {}, debug = false) => {
  const fnNames: string[] = Array.isArray(fnNameOrNames) ? fnNameOrNames : [fnNameOrNames]
  assert(fnNames.every(name => name.startsWith('test_')), "Solidity test functions must be prefixed with 'test_'")
