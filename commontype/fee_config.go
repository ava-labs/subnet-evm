...
type FeeConfig struct {
	...
	// If it is produced faster/slower, the block gas cost will be increased/decreased by the step value for each millisecond faster/slower than the target
...
func (f *FeeConfig) checkByteLens() error {
	...
	if isBiggerThanHashLen(new(big.Int).SetUint64(f.TargetBlockRate / 1000)) {
