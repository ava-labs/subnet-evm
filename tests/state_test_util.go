...
func (t *StateTest) RunNoVerify(subtest StateSubtest, vmconfig vm.Config, snapshotter bool, scheme string) (state StateTestState, root common.Hash, err error) {
	...
	rulesExtra := params.GetRulesExtra(config.Rules(block.Number(), params.IsMergeTODO, block.Time()))
	context := core.NewEVMBlockContext(rulesExtra.AvalancheRules, block.Header(), nil, &t.json.Env.Coinbase)
