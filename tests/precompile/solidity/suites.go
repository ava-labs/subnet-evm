...
// Registers the Synchronized Precompile Tests
// Before running the tests, this function creates the Avalanche locan network without explicitly creating any subnets,
// and then runs the hardhat tests for each one synchronously.
/*
func RegisterSyncTests() {
	...
	// ADD YOUR PRECOMPILE HERE
	ginkgo.It("your precompile", ginkgo.Label("Precompile"), ginkgo.Label("YourPrecompile"), ginkgo.Serial, func() {
		ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
		defer cancel()
		...
		// and the test file in ./contracts/tests/{your_precompile}.ts
		// If you want to use a different test command and genesis path than the defaults, you can
		// use the utils.RunTestCMD. See utils.CreateAndRunHardhatTests for an example.
		utils.CreateAndRunHardhatTests(ctx, "your_precompile")
	})
}
*/
