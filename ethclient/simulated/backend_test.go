...
func newTx(sim *Backend, key *ecdsa.PrivateKey) (*types.Transaction, error) {
	...
	nonce, err := client.AcceptedNonceAt(context.Background(), addr)
	if err != nil {
		return nil, err
	}
...
func TestForkResendTx(t *testing.T) {
	...
	// 3.
	// DO NOT MERGE: copied from Darioush's branch and needs discussion
	b, err := client.BlockByNumber(ctx, big.NewInt(int64(rpc.PendingBlockNumber)))
	require.NoError(t, err)
	require.Len(t, b.Transactions(), 1)
	require.Equal(t, tx.Hash(), b.Transactions()[0].Hash())
