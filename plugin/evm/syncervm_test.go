...
func createSyncServerAndClientVMs(t *testing.T, test syncTest, numBlocks int) *syncVMSetup {
	...
	server := newVM(t, testVMConfig{
		genesisJSON: genesisJSON(forkToChainConfig[upgradetest.Latest]),
	})
	...
	generateAndAcceptBlocks(t, server.vm, numBlocks, func(i int, gen *core.BlockGen) {
		...
		signedTx, err := types.SignTx(tx, types.NewEIP155Signer(server.vm.chainConfig.ChainID), testKeys[0].ToECDSA())
		require.NoError(err)
		gen.AddTx(signedTx)
	}, nil)
	...
	syncer.appSender.SendAppRequestF = func(ctx context.Context, nodeSet set.Set[ids.NodeID], requestID uint32, request []byte) error {
		...
		err := server.vm.AppRequest(ctx, nodeID, requestID, time.Now().Add(1*time.Second), request)
		require.NoError(err)
		return nil
	}
