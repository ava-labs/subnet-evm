...
type VM struct {
	...
	*chain.State
	...
	mockedValidatorState MockedValidatorState
	validatorMetaState   metastate.ValidatorMetastate
}
...
func (vm *VM) Initialize(
	...
	vm.mockedValidatorState = NewMockValidatorState(vm.ctx.ValidatorState)
	validatorsDB := prefixdb.New(validatorsDBPrefix, db)
	vm.validatorMetaState, err = metastate.NewValidatorMetaState(validatorsDB)
	if err != nil {
		return fmt.Errorf("failed to initialize validator metastate: %w", err)
	}
	// TODO: add a configuration to disable tracking uptime
	vm.uptimeManager = uptime.NewPausableManager(avalancheUptime.NewManager(vm.validatorMetaState, &vm.clock))
	...
	vm.validatorMetaState.RegisterListener(vm.uptimeManager)
...
func (vm *VM) onNormalOperationsStarted() error {
	...
	vdrIDs := vm.validatorMetaState.GetValidatorIDs().List()
...
func (vm *VM) Shutdown(ctx context.Context) error {
	...
	if vm.bootstrapped.Get() {
		...
		if err := vm.validatorMetaState.WriteValidatorMetadata(); err != nil {
			return fmt.Errorf("failed to write validator metadata: %w", err)
		}
	}
...
// performValidatorUpdate updates the validator metadata state with the current validator set
// and writes the metadata to the database.
func (vm *VM) performValidatorUpdate(ctx context.Context) error {
	now := time.Now()
	vm.logger.Debug("performing validator update")
	// get current validator set
	currentValidatorSet, err := vm.mockedValidatorState.GetCurrentValidatorSet(ctx, vm.ctx.SubnetID)
	if err != nil {
		return err
	}

	// load the current validator set into the validator metadata state
	if err := vm.loadCurrentValidators(currentValidatorSet); err != nil {
		return err
	}

	// write the validator metadata to the database
	if err := vm.validatorMetaState.WriteValidatorMetadata(); err != nil {
		return err
	}
...
func (vm *VM) loadCurrentValidators(vdrs map[ids.ID]*ValidatorOutput) error {
	metaState := vm.validatorMetaState
	currentValidationIDs := metaState.GetValidationIDs()
	...
	for vID := range currentValidationIDs {
		...
		if _, exists := vdrs[vID]; !exists {
			metaState.DeleteValidatorMetadata(vID)
		}
	}
	...
	for vID, vdr := range vdrs {
		if currentValidationIDs.Contains(vID) {
			...
			isActive, err := metaState.GetStatus(vID)
			if err != nil {
				return err
			}
			if isActive != vdr.IsActive {
				if err := metaState.SetStatus(vID, vdr.IsActive); err != nil {
					return err
				}
			}
		} else {
			err := metaState.AddNewValidatorMetadata(vdr.VID, vdr.NodeID, vdr.StartTime, vdr.IsActive)
			if err != nil {
				return err
			}
		}
