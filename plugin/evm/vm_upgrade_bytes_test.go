...
func TestVMUpgradeBytesPrecompile(t *testing.T) {
	...
	tvm := newVM(t, testVMConfig{
		...
		configJSON:  string(upgradeBytesJSON),
	})
	...
	if err := tvm.vm.Initialize(
		context.Background(), tvm.vm.ctx, tvm.vm.db, []byte(genesisJSONSubnetEVM), upgradeBytesJSON, []byte{}, tvm.toEngine, []*commonEng.Fx{}, tvm.appSender,
	); err != nil {
		t.Fatal(err)
	}
	defer func() {
...
func TestVMEupgradeActivatesCancun(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			genesisJSON: genesisJSON(forkToChainConfig[upgradetest.Etna]),
		...
		},
		{
			...
			genesisJSON: genesisJSON(forkToChainConfig[upgradetest.Durango]),
