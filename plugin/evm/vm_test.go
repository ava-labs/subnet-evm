...
type testVMConfig struct {
	...
	// fork. If fork is not available in upgradetest (e.g., SubnetEVM, PreSubnetEVM),
	// genesisJSON serves as a fallback to specify the genesis configuration directly.
	genesisJSON string
	configJSON  string
	// upgradeJSON specifies upgrade configuration for the VM
	upgradeJSON string
...
func newVM(t *testing.T, config testVMConfig) *testVM {
	...
	// - Otherwise, generate genesis JSON from the fork's chain config
	...
	for addr := range config.utxos {
		_, err := ids.ToID(hashing.ComputeHash256(addr.Bytes()))
		if err != nil {
...
func TestVMUpgrades(t *testing.T) {
	genesisTests := []struct {
		...
		{
			name:             "SubnetEVM",
			fork:             0, // Use genesisJSON directly
...
func TestSetPreferenceRace(t *testing.T) {
	...
	signedTx, err := types.SignTx(tx, types.NewEIP155Signer(tvm1.vm.chainConfig.ChainID), testKeys[0].ToECDSA())
	if err != nil {
		t.Fatal(err)
	}

	txErrors := tvm1.vm.txPool.AddRemotesSync([]*types.Transaction{signedTx})
	...
	for i := 0; i < 10; i++ {
		...
		signedTx, err := types.SignTx(tx, types.NewEIP155Signer(tvm1.vm.chainConfig.ChainID), testKeys[1].ToECDSA())
		if err != nil {
			t.Fatal(err)
		}
		txs[i] = signedTx
...
func TestReorgProtection(t *testing.T) {
	...
	if err := vm1BlkA.Verify(context.Background()); err != nil {
		t.Fatalf("Block failed verification on tvm1.vm: %s", err)
	}
	...
	vm2BlkA, err := tvm2.vm.ParseBlock(context.Background(), vm1BlkA.Bytes())
	if err != nil {
		t.Fatalf("Unexpected error parsing block from tvm2.vm: %s", err)
	}
	if err := vm2BlkA.Verify(context.Background()); err != nil {
		t.Fatalf("Block failed verification on tvm2.vm: %s", err)
	}
	if err := tvm2.vm.SetPreference(context.Background(), vm2BlkA.ID()); err != nil {
		t.Fatal(err)
	}
	...
	if err := vm1BlkA.Accept(context.Background()); err != nil {
		t.Fatalf("tvm1.vm failed to accept block: %s", err)
	}
	if err := vm2BlkA.Accept(context.Background()); err != nil {
		t.Fatalf("tvm2.vm failed to accept block: %s", err)
	}
	...
	// Create list of 10 successive transactions to build block A on tvm1.vm
	// and to be split into two separate blocks on tvm2.vm
	txs := make([]*types.Transaction, 10)
	for i := 0; i < 10; i++ {
		tx := types.NewTransaction(uint64(i), testEthAddrs[0], big.NewInt(10), 21000, big.NewInt(testMinGasPrice), nil)
		signedTx, err := types.SignTx(tx, types.NewEIP155Signer(tvm1.vm.chainConfig.ChainID), testKeys[1].ToECDSA())
	...
	// Add the remote transactions, build the block, and set tvm1.vm's preference for block A
	errs = tvm1.vm.txPool.AddRemotesSync(txs)
	for i, err := range errs {
		if err != nil {
			t.Fatalf("Failed to add transaction to tvm1.vm at index %d: %s", i, err)
		}
	}
	...
	// Split the transactions over two blocks, and set tvm2.vm's preference to them in sequence
	// after building each block
	// Block C
	errs = tvm2.vm.txPool.AddRemotesSync(txs[0:5])
	for i, err := range errs {
		if err != nil {
			t.Fatalf("Failed to add transaction to tvm2.vm at index %d: %s", i, err)
		}
	}
	...
	vm2BlkC, err := tvm2.vm.BuildBlock(context.Background())
	if err != nil {
		t.Fatalf("Failed to build BlkC on tvm2.vm: %s", err)
	}
...
func TestStickyPreference(t *testing.T) {
	...
	if err := vm2BlkC.Verify(context.Background()); err != nil {
		t.Fatalf("BlkC failed verification on tvm2.vm: %s", err)
	}
	...
	vm2BlkD, err := tvm2.vm.BuildBlock(context.Background())
	if err != nil {
		t.Fatalf("Failed to build BlkD on tvm2.vm: %s", err)
	}

	// Parse blocks produced in tvm2.vm
	vm1BlkC, err := tvm1.vm.ParseBlock(context.Background(), vm2BlkC.Bytes())
	if err != nil {
		t.Fatalf("Unexpected error parsing block from tvm2.vm: %s", err)
	}
	...
	if err := vm1BlkD.Accept(context.Background()); err != nil {
		t.Fatalf("Block failed acceptance on tvm1.vm: %s", err)
	}
...
func TestUncleBlock(t *testing.T) {
	...
	uncleBlock := tvm2.vm.newBlock(uncleEthBlock)

	if err := uncleBlock.Verify(context.Background()); !errors.Is(err, errUnclesUnsupported) {
		t.Fatalf("tvm2.vm should have failed with %q but got %q", errUnclesUnsupported, err.Error())
	}
	if _, err := tvm1.vm.ParseBlock(context.Background(), vm2BlkC.Bytes()); err != nil {
		t.Fatalf("tvm1.vm errored parsing blkC: %s", err)
	}
	if _, err := tvm1.vm.ParseBlock(context.Background(), uncleBlock.Bytes()); !errors.Is(err, errUnclesUnsupported) {
		t.Fatalf("tvm1.vm should have failed with %q but got %q", errUnclesUnsupported, err.Error())
	}
}
...
func TestAcceptReorg(t *testing.T) {
	...
	// Add the remote transactions, build the block, and set tvm1.vm's preference
	// for block B
	errs := tvm1.vm.txPool.AddRemotesSync(txs)
	for i, err := range errs {
		if err != nil {
...
func TestTxAllowListSuccessfulTx(t *testing.T) {
	...
	genesis := &core.Genesis{}
	if err := genesis.UnmarshalJSON([]byte(genesisJSON(forkToChainConfig[upgradetest.Durango]))); err != nil {
		t.Fatal(err)
	}
...
func TestVerifyManagerConfig(t *testing.T) {
	genesis := &core.Genesis{}
	require.NoError(t, genesis.UnmarshalJSON([]byte(genesisJSON(forkToChainConfig[upgradetest.Durango]))))
	...
	genesisJSONStr, err := genesis.MarshalJSON()
	require.NoError(t, err)

	vm := &VM{}
	ctx := snowtest.Context(t, snowtest.CChainID)
	// Use a fork that comes before Durango to test that managers cannot be added before Durango
	ctx.NetworkUpgrades = upgradetest.GetConfig(upgradetest.Banff) // Banff comes before Durango
	baseDB := memdb.New()
	atomicMemory := atomic.NewMemory(prefixdb.New([]byte{0}, baseDB))
	ctx.SharedMemory = atomicMemory.NewSharedMemory(ctx.ChainID)
	ctx.Lock.Lock()
...
func TestSkipChainConfigCheckCompatible(t *testing.T) {
	...
	require.NoError(t, json.Unmarshal([]byte(genesisJSON(forkToChainConfig[upgradetest.Durango])), genesisWithUpgrade))
	...
	err = reinitVM.Initialize(context.Background(), tvm.vm.ctx, tvm.vm.db, genesisWithUpgradeBytes, []byte{}, []byte{}, tvm.toEngine, []*commonEng.Fx{}, tvm.appSender)
	...
	err = reinitVM.Initialize(context.Background(), tvm.vm.ctx, tvm.vm.db, genesisWithUpgradeBytes, []byte{}, config, tvm.toEngine, []*commonEng.Fx{}, tvm.appSender)
	require.NoError(t, err)
...
func TestStandaloneDB(t *testing.T) {
	...
	err := vm.Initialize(
		...
		ctx,
		sharedDB,
		[]byte(genesisJSON(forkToChainConfig[upgradetest.Granite])), // use latest fork for testing
		nil,
		[]byte(configJSON),
		issuer,
		[]*commonEng.Fx{},
...
func TestFeeManagerRegressionMempoolMinFeeAfterRestart(t *testing.T) {
	...
	restartedVM, err := restartVM(tvm.vm, tvm.vm.db, []byte(genesisJSON), tvm.toEngine, tvm.appSender, true)
	require.NoError(t, err)
	...
	restartedVM, err = restartVM(restartedVM, tvm.vm.db, []byte(genesisJSON), tvm.toEngine, tvm.appSender, true)
	require.NoError(t, err)
	newTxPoolHeadChan = make(chan core.NewTxPoolReorgEvent, 1)
	restartedVM.txPool.SubscribeNewReorgEvent(newTxPoolHeadChan)
