...
func TestVMConfig(t *testing.T) {
	...
	vm := newVM(t, testVMConfig{
		configJSON: fmt.Sprintf(`{"rpc-tx-fee-cap": %g,"eth-apis": %s}`, txFeeCap, enabledEthAPIs[0]),
	}).vm
...
func TestBuildEthTxBlock(t *testing.T) {
	...
	blk1 := issueAndAccept(t, tvm.toEngine, tvm.vm)
	...
	blk2 := issueAndAccept(t, tvm.toEngine, tvm.vm)
	newHead = <-newTxPoolHeadChan
...
func TestSetPreferenceRace(t *testing.T) {
	...
	vm1, issuer1 := tvm1.vm, tvm1.toEngine
	vm2, issuer2 := tvm2.vm, tvm2.toEngine
...
func TestFeeManagerRegressionMempoolMinFeeAfterRestart(t *testing.T) {
	...
	restartedVM, err := restartVM(tvm.vm, tvm.db, genesisJSON, tvm.toEngine, tvm.appSender, true)
	require.NoError(t, err)
	...
	errs = restartedVM.txPool.AddRemotesSync([]*types.Transaction{signedTx})
	require.NoError(t, errs[0])
	blk := issueAndAccept(t, tvm.toEngine, restartedVM)
	newHead := <-newTxPoolHeadChan
	require.Equal(t, newHead.Head.Hash(), common.Hash(blk.ID()))
	// Contract is initialized but no preconfig is given, reader should return genesis fee config
	...
	restartedVM, err = restartVM(restartedVM, tvm.db, genesisJSON, tvm.toEngine, tvm.appSender, true)
	require.NoError(t, err)
	newTxPoolHeadChan = make(chan core.NewTxPoolReorgEvent, 1)
	restartedVM.txPool.SubscribeNewReorgEvent(newTxPoolHeadChan)
