...
func testVMConfig(t *testing.T, c VmTestManager) {
	...
	workerConfig := GenesisVMConfig{false, "",configJSON, "",
	}
	vmb, err := c.Create(t, workerConfig)
	require.NoError(err)
	defer func() {
		require.NoError(vmb.Shutdown(context.Background()))
	}()

	config, err := vmb.GetVMConfig()
	require.NoError(err)
	require.Equal(txFeeCap, config.RPCTxFeeCap)
	require.Equal(config.EthAPIs(), enabledEthAPIs)
}
