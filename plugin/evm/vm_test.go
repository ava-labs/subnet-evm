...
func testBuildEthTxBlock(t *testing.T, scheme string) {
	...
	signedTx := newSignedLegacyTx(t, tvm.vm.chainConfig, testKeys[0].ToECDSA(), 0, &testEthAddrs[1], big.NewInt(1), 21000, big.NewInt(ethparams.InitialBaseFee), nil)
	...
	for i := 0; i < 10; i++ {
		signedTx := newSignedLegacyTx(t, tvm.vm.chainConfig, testKeys[1].ToECDSA(), uint64(i), &testEthAddrs[1], big.NewInt(10), 21000, big.NewInt(testMinGasPrice), nil)
		txs[i] = signedTx
	}
...
func testSetPreferenceRace(t *testing.T, scheme string) {
	...
	signedTx := newSignedLegacyTx(t, vm1.chainConfig, testKeys[0].ToECDSA(), 0, &testEthAddrs[1], big.NewInt(1), 21000, big.NewInt(testMinGasPrice), nil)
	vm1BlkA, err := IssueTxsAndSetPreference([]*types.Transaction{signedTx}, vm1)
	if err != nil {
	...
	txs := make([]*types.Transaction, 10)
	for i := 0; i < 10; i++ {
		signedTx := newSignedLegacyTx(t, vm1.chainConfig, testKeys[1].ToECDSA(), uint64(i), &testEthAddrs[1], big.NewInt(10), 21000, big.NewInt(testMinGasPrice), nil)
		txs[i] = signedTx
	}
...
func testNonCanonicalAccept(t *testing.T, scheme string) {
	...
	signedTx := newSignedLegacyTx(t, vm1.chainConfig, testKeys[0].ToECDSA(), 0, &testEthAddrs[1], big.NewInt(1), 21000, big.NewInt(testMinGasPrice), nil)
	vm1BlkA, err := IssueTxsAndBuild([]*types.Transaction{signedTx}, vm1)
	if err != nil {
		t.Fatalf("Failed to build block with import transaction: %s", err)
	}
...
func testFutureBlock(t *testing.T, scheme string) {
	...
	signedTx := newSignedLegacyTx(t, tvm.vm.chainConfig, testKeys[0].ToECDSA(), 0, &testEthAddrs[1], big.NewInt(1), 21000, big.NewInt(testMinGasPrice), nil)
	blkA, err := IssueTxsAndBuild([]*types.Transaction{signedTx}, tvm.vm)
	if err != nil {
		t.Fatalf("Failed to build block with import transaction: %s", err)
	}
...
func testBuildSubnetEVMBlock(t *testing.T, scheme string) {
	...
	signedTx := newSignedLegacyTx(t, tvm.vm.chainConfig, testKeys[0].ToECDSA(), 0, &testEthAddrs[1], big.NewInt(1), 21000, big.NewInt(ethparams.big.NewInt(ethparams.InitialBaseFee)), nil)
	...
	txs := make([]*types.Transaction, 10)
	for i := 0; i < 5; i++ {
		signedTx := newSignedLegacyTx(t, tvm.vm.chainConfig, key, uint64(i), &address, big.NewInt(10), 21000, big.NewInt(testMinGasPrice), nil)
		txs[i] = signedTx
	}
	for i := 5; i < 10; i++ {
		signedTx := newSignedLegacyTx(t, tvm.vm.chainConfig, key, uint64(i), &address, big.NewInt(10), 21000, big.NewInt(testMinGasPrice), nil)
...
func TestParentBeaconRootBlock(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			signedTx := newSignedLegacyTx(t, tvm.vm.chainConfig, testKeys[0].ToECDSA(), 0, &testEthAddrs[1], big.NewInt(1), 21000, big.NewInt(ethparams.InitialBaseFee), nil)
			blk, err := IssueTxsAndBuild([]*types.Transaction{signedTx}, tvm.vm)
			if err != nil {
				t.Fatalf("Failed to build block with import transaction: %s", err)
			}
	...
	for _, testCase := range []struct {
		...
		},
		{
			...
			testCase: func(t *testing.T, vm *VM) {
				...
				signedTx := newSignedLegacyTx(t, vm.chainConfig, testKeys[0].ToECDSA(), 0, &testEthAddrs[1], big.NewInt(1), 21000, big.NewInt(ethparams.InitialBaseFee), nil)
		...
		},
		{
			...
			testCase: func(t *testing.T, vm *VM) {
				...
				signedTx = newSignedLegacyTx(t, vm.chainConfig, testKeys[0].ToECDSA(), 1, &testEthAddrs[1], big.NewInt(1), 21000, big.NewInt(ethparams.InitialBaseFee), nil)
		...
		},
		{
			...
			testCase: func(t *testing.T, vm *VM) {
				...
				blk, err := IssueTxsAndBuild([]*types.Transaction{signedTx}, vm)
				require.NoError(t, err)
				require.NoError(t, blk.Accept(context.Background()))
				signedTx = newSignedLegacyTx(t, vm.chainConfig, testKeys[0].ToECDSA(), 1, &testEthAddrs[1], big.NewInt(1), 21000, big.NewInt(ethparams.InitialBaseFee), nil)
...
func TestDelegatePrecompile_BehaviorAcrossUpgrades(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			deployGasPrice: big.NewInt(ethparams.InitialBaseFee),
			txGasPrice:     big.NewInt(ethparams.InitialBaseFee),
