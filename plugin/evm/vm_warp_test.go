...
func TestSendWarpMessage(t *testing.T) {
	...
	require.NoError(genesis.UnmarshalJSON([]byte(genesisJSON(forkToChainConfig[upgradetest.Durango]))))
...
func TestReceiveWarpMessage(t *testing.T) {
	...
	tvm := newVM(t, testVMConfig{
		genesisJSON: string(genesisJSON),
		configJSON:  string(upgradeBytes),
	})
...
func testReceiveWarpMessage(
	t *testing.T, toEngine chan commonEng.Message, vm *VM,
	sourceChainID ids.ID,
