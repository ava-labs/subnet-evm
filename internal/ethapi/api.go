...
func (api *EthereumAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
	gasPrice, err := api.b.SuggestPrice(ctx)
	if err != nil {
		return nil, err
	}
...
// it is produced now.
func (api *EthereumAPI) BaseFee(ctx context.Context) (*hexutil.Big, error) {
	baseFee, err := api.b.EstimateBaseFee(ctx)
	if err != nil {
		return nil, err
	}
...
func (api *TxPoolAPI) Content() map[string]map[string]map[string]*RPCTransaction {
	...
	estimatedBaseFee, _ := api.b.EstimateBaseFee(context.Background())
	// Flatten the pending transactions
	for account, txs := range pending {
		dump := make(map[string]*RPCTransaction)
		for _, tx := range txs {
			dump[fmt.Sprintf("%d", tx.Nonce())] = NewRPCTransaction(tx, curHeader, estimatedBaseFee, api.b.ChainConfig())
		}
...
func (api *TransactionAPI) GetTransactionByHash(ctx context.Context, hash common.Hash) (*RPCTransaction, error) {
	...
	if !found {
		...
		if tx := api.b.GetPoolTransaction(hash); tx != nil {
			estimatedBaseFee, _ := api.b.EstimateBaseFee(ctx)
			return NewRPCTransaction(tx, api.b.CurrentHeader(), estimatedBaseFee, api.b.ChainConfig()), nil
		}
		if err == nil {
			return nil, nil
		}
...
func (api *TransactionAPI) PendingTransactions() ([]*RPCTransaction, error) {
	...
	for _, tx := range pending {
		...
		if _, exists := accounts[from]; exists {
			estimatedBaseFee, _ := api.b.EstimateBaseFee(context.Background())
			transactions = append(transactions, NewRPCTransaction(tx, curHeader, estimatedBaseFee, api.b.ChainConfig()))
		}
	}
