...
// ADD YOUR PRECOMPILE HERE
var precompileKeys = []precompileKey{contractDeployerAllowListKey, contractNativeMinterKey, txAllowListKey, feeManagerKey, rewardManagerKey, warpMessengerKey /* {yourPrecompile}Key */}
...
func (p *PrecompileUpgrade) getByKey(key precompileKey) (precompile.StatefulPrecompileConfig, bool) {
	...
	case warpMessengerKey:
		return p.WarpMessengerConfig, p.WarpMessengerConfig != nil
	// ADD YOUR PRECOMPILE HERE
	/*
		case {yourPrecompile}Key:
...
// GetWarpMessengerConfig returns the latest forked WarpMessengerConfig
// specified by [c] or nil if it was never enabled.
func (c *ChainConfig) GetWarpMessengerConfig(blockTimestamp *big.Int) *precompile.WarpMessengerConfig {
	if val := c.getActivePrecompileConfig(blockTimestamp, warpMessengerKey, c.PrecompileUpgrades); val != nil {
		return val.(*precompile.WarpMessengerConfig)
	}
	return nil
}
...
func (c *ChainConfig) GetActivePrecompiles(blockTimestamp *big.Int) PrecompileUpgrade {
	...
	if config := c.GetWarpMessengerConfig(blockTimestamp); config != nil && !config.Disable {
		pu.WarpMessengerConfig = config
	}
	// ADD YOUR PRECOMPILE HERE
