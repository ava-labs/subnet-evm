...
// IsWarpMessenger returns whether [blockTimestamp] is either equal to the WarpMessenger fork block timestamp or greater.
func (c *ChainConfig) IsWarpMessenger(blockTimestamp *big.Int) bool {
	config := c.GetWarpMessengerConfig(blockTimestamp)
	return config != nil && !config.Disable
}
...
func (c *ChainConfig) AvalancheRules(blockNum, blockTimestamp *big.Int) Rules {
	...
	rules.IsWarpMessengerEnabled = c.IsWarpMessenger(blockTimestamp)
	// ADD YOUR PRECOMPILE HERE
