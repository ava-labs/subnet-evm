...
const (
	...
	MetricsEndpointsKey = "metrics-endpoints"
)
...
type Config struct {
	...
	BlockchainIDStr  string        `json:"blockchain-id-str"`
	MetricsEndpoints []string      `json:"metrics-endpoints"`
}
...
func BuildConfig(v *viper.Viper) (Config, error) {
	c := Config{
		...
		MetricsEndpoints: v.GetStringSlice(MetricsEndpointsKey),
	}
...
func addSimulatorFlags(fs *pflag.FlagSet) {
	...
	fs.String(BlockchainIDStrKey, "C", "Specify the blockchain ID for the chain you are executing the simulation on. Ex: C, X, P, pUSPHTrvs8ASVfPjrf9xdQ6SwDFhSmCZ3EKuCUMASHky2Wvr (full 32 byte hash blockchainID)")
	fs.StringSlice(MetricsEndpointsKey, []string{}, "Specify a comma separated list of metric endpoints for extra logging at the end of the simulation (minimum of 1 endpoint)")
}
